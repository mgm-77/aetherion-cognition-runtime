...
VERSION = 'v2.6'
PACK_ROOT_NAME = 'morph_runtime_core_v2_6'
...
MODULES_BY_TASK = {
    'math': [
        ...
        'PackValidator','IntegrityGuard','FabricOrchestratorV26','ReportBuilder'
    ],
    'simple': [
        'RuntimeKernel','RuntimeStateManager','RuntimeIndexManager','ContinuityPackManager','SessionStateAdapter',
        'RecoveryEngine','AutoRepairBasic','OrphanHandler','ComparativeResumeEngine','HandoffExporter',
...
def build_quick_report(summary: Dict[str, Any], integrity: Dict[str, Any], repair_summary: Dict[str, Any]) -> str:
    ...
    return '\n'.join([
        ...
        "Upload-ready reports: report_v2.6.txt / report_v2.6.md / report_v2.6.pdf",
    ]) + '\n'
...
def export_custom(path_source: Path, target: Optional[Path]) -> Optional[Path]:
    if target is None:
        return None
    target.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy2(path_source, target)
    return target
...
def main() -> None:
    ...
    if runtime_index_file.exists():
        ...
        if not args.fresh_if_mising if False else False:
            pass
    ...
    shutil.copy2(report_txt_file, final_bundle / report_txt_file.name)
    shutil.copy2(report_md_file, final_bundle / report_md_file.name)
    shutil.copy2(report_pdf_file, final_bundle / report_pdf_file.name)
    shutil.copy2(manifest_file, final_bundle / manifest_file.name)
    shutil.copy2(integrity_file, final_bundle / integrity_file.name)
    shutil.copy2(quick_report_file, final_bundle / quick_report_file.name)
    write_text(final_bundle / 'UPLOAD_README.txt', '\n'.join([
        f'MORPH Runtime Core {VERSION}',
        f'Project: {args.project_id}',
        'Key features: recovery engine / auto-repair basic / orphan handling'
    ]) + '\n')
    ...
    print(json.dumps(repair_summary, indent=2, ensure_ascii=False))
    print('')
