...
VERSION = 'v3.4'
PACK_ROOT_NAME = 'morph_runtime_core_v3_4'
...
MODULES_BY_TASK = {
    'math': [
        ...
        'QuickResumePack','SummaryPackBuilder','PackValidator','IntegrityGuard','FabricOrchestratorV34',
        ...
        'PressureSignalBuilder','DriftNotesBuilder'
    ],
    'simple': [
        'RuntimeKernel','RuntimeStateManager','RuntimeIndexManager','ContinuityPackManager','SessionStateAdapter',
        'RecoveryEngine','AutoRepairBasic','OrphanHandler','ComparativeResumeEngine','HandoffExporter',
...
class MorphRuntimeCoreV34:
    def __init__(self, base_dir: Path):
        ...
        self.branch_compare_file = self.project_dir / 'branch_compare_v3_4.json'
        self.branch_verdict_md_file = self.project_dir / 'branch_verdict_v3_4.md'
        self.branch_verdict_txt_file = self.project_dir / 'branch_verdict_v3_4.txt'
        self.runtime_dashboard_json = self.project_dir / 'runtime_dashboard_v3_4.json'
        self.runtime_dashboard_txt = self.project_dir / 'runtime_dashboard_v3_4.txt'
        self.runtime_dashboard_md = self.project_dir / 'runtime_dashboard_v3_4.md'
        self.archive_summary_file = self.project_dir / 'archive_summary.json'
        self.release_summary_json = self.project_dir / 'release_summary_v3_4.json'
        self.release_summary_txt = self.project_dir / 'release_summary_v3_4.txt'
        self.report_txt = self.project_dir / 'report_v3.4.txt'
        self.report_md = self.project_dir / 'report_v3.4.md'
        self.session_query_json = self.project_dir / 'session_query_v3_4.json'
        self.session_query_txt = self.project_dir / 'session_query_v3_4.txt'
        self.timeline_json = self.project_dir / 'history_timeline_v3_4.json'
        self.timeline_txt = self.project_dir / 'history_timeline_v3_4.txt'
        self.timeline_md = self.project_dir / 'history_timeline_v3_4.md'
        self.pressure_json = self.project_dir / 'pressure_signals_v3_4.json'
        self.pressure_txt = self.project_dir / 'pressure_signals_v3_4.txt'
        self.pressure_md = self.project_dir / 'pressure_signals_v3_4.md'
        self.drift_json = self.project_dir / 'drift_notes_v3_4.json'
        self.drift_txt = self.project_dir / 'drift_notes_v3_4.txt'
        self.drift_md = self.project_dir / 'drift_notes_v3_4.md'
    ...
    def build_compact_handoff(self):
        ...
        data = {
            ...
            'resume_main': f"python morph_runtime_core_v3_4.py --append-task math --resume-session {latest['session_id']}" if latest else None,
            'resume_simple': "python morph_runtime_core_v3_4.py --append-task simple --branch simple_track" if latest else None,
        }
    ...
    def build_quick_resume_pack(self):
        ...
        data = {
            ...
            'main_resume': f"python morph_runtime_core_v3_4.py --append-task math --resume-session {latest['session_id']}" if latest and latest['task_type'] == 'math' else None,
            'simple_resume': "python morph_runtime_core_v3_4.py --append-task simple --branch simple_track",
            'checkpoint_create': "python morph_runtime_core_v3_4.py --checkpoint-label stable_v3_4",
    ...
    def branch_verdicts(self):
        ...
        md_lines = ['# Branch Verdicts v3.4', '']
        txt_lines = ['Branch Verdicts v3.4', '']
    ...
    def build_release_summary(self):
        ...
        data = {
            ...
            'files': {
                ...
                'drift_txt': str(self.drift_txt),
                'drift_json': str(self.drift_json),
            }
        }
        ...
        lines = [
            ...
            f'Drift TXT: {self.drift_txt.name}',
    ...
    def build_timeline(self):
        ...
        md = ['# History Timeline v3.4', '', f'- **Project:** {self.project_id}', f'- **Entry count:** `{len(entries)}`', '', '## Entries']
        for e in entries:
            if e['type'] == 'session':
    ...
    def build_pressure_signals(self):
        ...
        md = ['# Pressure Signals v3.4', '', f'- **Project:** {self.project_id}', f'- **Signal count:** `{len(signals)}`', '', '## Signals']
        for s in signals:
    ...
    def build_drift_notes(self):
        ...
        for n in notes:
            lines.append(
                f"- {n['branch']} | start={n['start_session_id']} | end={n['end_session_id']} | "
                f"route_start={n['route_start']} | route_end={n['route_end']} | "
                f"confidence_delta={n['confidence_delta']} | health_delta={n['health_delta']} | "
                f"deferred_delta={n['deferred_delta']} | verdict={n['verdict']}"
            )
        write_text(self.drift_txt, '\n'.join(lines) + '\n')
        md = ['# Drift Notes v3.4', '', f'- **Project:** {self.project_id}', f'- **Branch count:** `{len(notes)}`', '', '## Notes']
        for n in notes:
            md.append(
                f"- `{n['branch']}` — start=`{n['start_session_id']}` | end=`{n['end_session_id']}` | "
                f"route_start=`{n['route_start']}` | route_end=`{n['route_end']}` | "
    ...
    def build_upload_reports(self):
        ...
        txt_lines = [
            ...
            'V3.4 features:',
            '- drift notes',
            '- branch stability / drift verdicts',
            '- drift export txt / json / md',
        ]
        write_text(self.report_txt, '\n'.join(txt_lines) + '\n')
        write_text(self.report_md, '# Report v3.4\n\n' + '\n'.join(f'- {line}' for line in txt_lines if line.strip()) + '\n')
    ...
    def print_summary(self, session, root_created):
        print('=== MORPH Runtime Core v3.4 / Summary ===')
        ...
        print(f'Drift txt:      {self.drift_txt}')
        print(f'Drift md:       {self.drift_md}')
        print(f'Drift json:     {self.drift_json}')
    ...
    def cli(self):
        parser = argparse.ArgumentParser(description='MORPH Runtime Core v3.4')
...
def main():
    MorphRuntimeCoreV34(Path.cwd()).cli()
