"""
MORPH Runtime Core v0.9
Conflict Resolver + Queue Arbitration

Adds on top of v0.8:
- ConflictResolver
- QueueArbitrationLayer
- horizon promotion / demotion
- arbitration trace between short / medium / deep
- explicit conflict decisions
- pressure + confidence aware queue movement
...
class VerificationTrigger:
    ...
    def should_verify(task_type: TaskType, difficulty: Difficulty, chosen_depth: int, budget: BudgetProfile, chunk: str, historical_record: Optional[RouteMemoryRecord] = None, chosen_route: Optional[CandidateRoute] = None, residue_bundles: Optional[List[ResidueBundle]] = None) -> bool:
        ...
        if residue_bundles:
            ...
            if rv > 0.7 and difficulty != Difficulty.EASY:
                return True
        return False
...
class MultiScaleScheduler:
    ...
    def build(plan: HorizonPlan, deferred_items: List[DeferredWorkItem]) -> MultiScaleSchedule:
        ...
        deferred_sorted = sorted(deferred_items, key=lambda x: (x.priority, x.reuse_count), reverse=True)
        for item in deferred_sorted:
            label = f"deferred::{item.item_id}"
            if item.horizon == Horizon.IMMEDIATE.value:
                schedule.immediate_queue.append(label)
...
class SchedulerPressureEngine:
    ...
    def rebalance(schedule: MultiScaleSchedule, pressure: Dict[str, float]) -> MultiScaleSchedule:
        ...
        if pressure.get(Horizon.MEDIUM.value, 0) > 0.4 and "residue_alignment_pass" not in schedule.medium_queue:
            schedule.medium_queue.append("residue_alignment_pass")
        return schedule
...
class ConflictResolver:
    ...
    def resolve(schedule: MultiScaleSchedule, pressure: Dict[str, float], route_confidence: float) -> List[ArbitrationDecision]:
        ...
        # Promote deep work if deep pressure dominates and confidence is high.
        if pressure.get(Horizon.DEEP.value, 0) > pressure.get(Horizon.SHORT.value, 0) and route_confidence >= 0.9:
            if "core_claim_pass" in schedule.short_queue:
                ...
                decisions.append(ArbitrationDecision("core_claim_pass", Horizon.SHORT.value, Horizon.DEEP.value, "deep_pressure_dominates_with_high_confidence"))

        # Promote residue alignment if medium pressure dominates.
        if pressure.get(Horizon.MEDIUM.value, 0) > 0.4 and "structural_consistency_pass" in schedule.medium_queue:
            # keep it there, but mark conflict resolution implicitly by adding a companion pass if absent
            if "residue_alignment_pass" not in schedule.medium_queue:
                schedule.medium_queue.append("residue_alignment_pass")
                decisions.append(ArbitrationDecision("residue_alignment_pass", Horizon.MEDIUM.value, Horizon.MEDIUM.value, "medium_pressure_support"))

        # Keep deferred scan in short when short pressure is strong.
        if pressure.get(Horizon.SHORT.value, 0) > 0.45 and "deferred_reuse_scan" not in schedule.short_queue:
            schedule.short_queue.append("deferred_reuse_scan")
            decisions.append(ArbitrationDecision("deferred_reuse_scan", Horizon.SHORT.value, Horizon.SHORT.value, "short_pressure_support"))
...
class QueueArbitrationLayer:
    ...
    def apply(schedule: MultiScaleSchedule, decisions: List[ArbitrationDecision]) -> MultiScaleSchedule:
        # decisions already mutate schedule in ConflictResolver; this layer is explicit for architecture clarity
        return schedule


class FabricAssemblyEngine:
    @staticmethod
    def build_fabric(prompt: str, context_items: List[ContextItem], budget: BudgetProfile, route_memory: RouteMemoryEngine, residue_registry: ResidueRegistry, deferred_registry: DeferredWorkRegistry):
...
class FabricOrchestratorV09:
    def __init__(self, budget: Optional[BudgetProfile] = None, route_memory: Optional[RouteMemoryEngine] = None, residue_registry: Optional[ResidueRegistry] = None, deferred_registry: Optional[DeferredWorkRegistry] = None):
    ...
    def run(self, prompt: str, context_items: List[ContextItem]) -> RuntimeState:
        ...
        state.active_modules = [
            "FabricAssemblyEngine","FabricOrchestratorV09","FabricHealthMonitor","HorizonPlanner","MultiScaleScheduler","SchedulerPressureEngine",
            "ConflictResolver","QueueArbitrationLayer","DeferredWorkRegistry","PromptClassifier","TokenDifficultyEstimator","DepthGovernor","MemoryTileManager",
            "RouteMemoryEngine","ResidueRegistry","CandidateRouteGenerator","BranchScoringLayer","EarlyCollapseController",
        ]
        ...
        state.fabric_state.phase = FabricPhase.MULTI_SCALE_SCHEDULING.value
        state.log("fabric_phase", phase=FabricPhase.MULTI_SCALE_SCHEDULING.value)
        state.schedule = MultiScaleScheduler.build(state.horizon_plan, deferred_items)

        state.fabric_state.phase = FabricPhase.ADAPTIVE_REBALANCING.value
        state.log("fabric_phase", phase=FabricPhase.ADAPTIVE_REBALANCING.value)
        ...
        state.log("adaptive_pressure", pressure=pressure)

        state.fabric_state.phase = FabricPhase.CONFLICT_ARBITRATION.value
        state.log("fabric_phase", phase=FabricPhase.CONFLICT_ARBITRATION.value)
        ...
        state.fabric_state.schedule_loads = {
            Horizon.IMMEDIATE.value: len(state.schedule.immediate_queue),
            Horizon.SHORT.value: len(state.schedule.short_queue),
            Horizon.MEDIUM.value: len(state.schedule.medium_queue),
            Horizon.DEEP.value: len(state.schedule.deep_queue),
        }
        state.log("arbitration_decisions", decisions=state.arbitration_trace)
        state.log("multi_scale_schedule", schedule=asdict(state.schedule))
        ...
        if len(chunks) == 1:
            chunk_horizons = [Horizon.DEEP]
        elif len(chunks) == 2:
            chunk_horizons = [Horizon.SHORT, Horizon.DEEP]
        else:
        ...
        for idx, chunk in enumerate(chunks):
            ...
            should_defer = False
            if horizon == Horizon.DEEP and difficulty in {Difficulty.NORMAL, Difficulty.CRITICAL} and self.budget.deferred_trigger_on_deep_normal:
                should_defer = True
            if horizon == Horizon.MEDIUM and difficulty in {Difficulty.NORMAL, Difficulty.CRITICAL} and self.budget.deferred_trigger_on_medium_normal:
                should_defer = True

            if should_defer:
                base_priority = 0.8 if horizon == Horizon.DEEP else 0.6
...
def demo() -> None:
    ...
    print("=== MORPH Runtime Core v0.9 Demo / Run 1 ===")
    ...
    print(f"Horizon pressure: {r1.fabric_state.horizon_pressure}")
    ...
    print(f"Deferred items used: {len(r1.deferred_items_used)}")

    print("\n=== MORPH Runtime Core v0.9 Demo / Run 2 ===")
    ...
    print(f"Horizon pressure: {r2.fabric_state.horizon_pressure}")
    ...
    print("\n=== Arbitration Trace ===")
    print(json.dumps(r2.arbitration_trace, indent=2))
    print("\n=== Fabric State Snapshot ===")
    print(json.dumps(asdict(r2.fabric_state), indent=2))
    print("\n=== Multi-Scale Schedule Snapshot ===")
    print(json.dumps(asdict(r2.schedule), indent=2))
