...
VERSION = 'v3.6'
PACK_ROOT_NAME = 'morph_runtime_core_v3_6'
...
MODULES_BY_TASK = {
    'math': [
        ...
        'QuickResumePack','SummaryPackBuilder','PackValidator','IntegrityGuard','FabricOrchestratorV36',
...
class MorphRuntimeCoreV36:
    def __init__(self, base_dir: Path):
        ...
        self.branch_compare_file = self.project_dir / 'branch_compare_v3_6.json'
        self.branch_verdict_md_file = self.project_dir / 'branch_verdict_v3_6.md'
        self.branch_verdict_txt_file = self.project_dir / 'branch_verdict_v3_6.txt'
        self.runtime_dashboard_json = self.project_dir / 'runtime_dashboard_v3_6.json'
        self.runtime_dashboard_txt = self.project_dir / 'runtime_dashboard_v3_6.txt'
        self.runtime_dashboard_md = self.project_dir / 'runtime_dashboard_v3_6.md'
        self.archive_summary_file = self.project_dir / 'archive_summary.json'
        self.release_summary_json = self.project_dir / 'release_summary_v3_6.json'
        self.release_summary_txt = self.project_dir / 'release_summary_v3_6.txt'
        self.report_txt = self.project_dir / 'report_v3.6.txt'
        self.report_md = self.project_dir / 'report_v3.6.md'
        self.session_query_json = self.project_dir / 'session_query_v3_6.json'
        self.session_query_txt = self.project_dir / 'session_query_v3_6.txt'
        self.timeline_json = self.project_dir / 'history_timeline_v3_6.json'
        self.timeline_txt = self.project_dir / 'history_timeline_v3_6.txt'
        self.timeline_md = self.project_dir / 'history_timeline_v3_6.md'
        self.pressure_json = self.project_dir / 'pressure_signals_v3_6.json'
        self.pressure_txt = self.project_dir / 'pressure_signals_v3_6.txt'
        self.pressure_md = self.project_dir / 'pressure_signals_v3_6.md'
        self.drift_json = self.project_dir / 'drift_notes_v3_6.json'
        self.drift_txt = self.project_dir / 'drift_notes_v3_6.txt'
        self.drift_md = self.project_dir / 'drift_notes_v3_6.md'
        self.checkpoint_diff_json = self.project_dir / 'checkpoint_diff_v3_6.json'
        self.checkpoint_diff_txt = self.project_dir / 'checkpoint_diff_v3_6.txt'
        self.checkpoint_diff_md = self.project_dir / 'checkpoint_diff_v3_6.md'
        self.checkpoint_registry_json = self.project_dir / 'checkpoint_registry_v3_6.json'
        self.checkpoint_registry_txt = self.project_dir / 'checkpoint_registry_v3_6.txt'
        self.checkpoint_registry_md = self.project_dir / 'checkpoint_registry_v3_6.md'
    ...
    def build_compact_handoff(self):
        ...
        data = {
            ...
            'resume_main': f"python morph_runtime_core_v3_6.py --append-task math --resume-session {latest['session_id']}" if latest else None,
            'resume_simple': "python morph_runtime_core_v3_6.py --append-task simple --branch simple_track" if latest else None,
        }
    ...
    def build_quick_resume_pack(self):
        ...
        data = {
            ...
            'main_resume': f"python morph_runtime_core_v3_6.py --append-task math --resume-session {latest['session_id']}" if latest and latest['task_type'] == 'math' else None,
            'simple_resume': "python morph_runtime_core_v3_6.py --append-task simple --branch simple_track",
            'checkpoint_create': "python morph_runtime_core_v3_6.py --checkpoint-label stable_v3_6",
    ...
    def branch_verdicts(self):
        ...
        md_lines = ['# Branch Verdicts v3.6', '']
        txt_lines = ['Branch Verdicts v3.6', '']
    ...
    def build_release_summary(self):
        ...
        data = {
            ...
            'files': {
                ...
                'checkpoint_diff_json': str(self.checkpoint_diff_json),
                'checkpoint_registry_txt': str(self.checkpoint_registry_txt),
                'checkpoint_registry_json': str(self.checkpoint_registry_json),
            }
        }
        ...
        lines = [
            ...
            f'Checkpoint Diff TXT: {self.checkpoint_diff_txt.name}',
    ...
    def build_timeline(self):
        ...
        md = ['# History Timeline v3.6', '', f'- **Project:** {self.project_id}', f'- **Entry count:** `{len(entries)}`', '', '## Entries']
        for e in entries:
            if e['type'] == 'session':
    ...
    def build_pressure_signals(self):
        ...
        md = ['# Pressure Signals v3.6', '', f'- **Project:** {self.project_id}', f'- **Signal count:** `{len(signals)}`', '', '## Signals']
        for s in signals:
    ...
    def build_drift_notes(self):
        ...
        md = ['# Drift Notes v3.6', '', f'- **Project:** {self.project_id}', f'- **Branch count:** `{len(notes)}`', '', '## Notes']
        for n in notes:
    ...
    def build_checkpoint_registry(self):
        ...
        md = ['# Checkpoint Registry v3.6', '', f'- **Project:** {self.project_id}', f'- **Checkpoint count:** `{len(entries)}`', '', '## Checkpoints']
        for e in entries:
    ...
    def build_checkpoint_diff(self, left_label=None, right_label=None):
        ...
        if left_meta and right_meta:
            ...
            diff = {
                'left_checkpoint': left_meta['label'],
                'right_checkpoint': right_meta['label'],
    ...
    def build_upload_reports(self):
        ...
        txt_lines = [
            ...
            '- diff between explicit labels',
            '- checkpoint registry export txt / json / md',
        ]
        write_text(self.report_txt, '\n'.join(txt_lines) + '\n')
        write_text(self.report_md, '# Report v3.6\n\n' + '\n'.join(f'- {line}' for line in txt_lines if line.strip()) + '\n')
    ...
    def print_summary(self, session, root_created):
        print('=== MORPH Runtime Core v3.6 / Summary ===')
        ...
        print(f'Registry txt:   {self.checkpoint_registry_txt}')
        print(f'Diff txt:       {self.checkpoint_diff_txt}')
    ...
    def cli(self):
        parser = argparse.ArgumentParser(description='MORPH Runtime Core v3.6')
...
def main():
    MorphRuntimeCoreV36(Path.cwd()).cli()
