...
VERSION = 'v3.2'
PACK_ROOT_NAME = 'morph_runtime_core_v3_2'
...
MODULES_BY_TASK = {
    'math': [
        ...
        'QuickResumePack','SummaryPackBuilder','PackValidator','IntegrityGuard','FabricOrchestratorV32',
...
class MorphRuntimeCoreV32:
    def __init__(self, base_dir: Path):
        ...
        self.branch_compare_file = self.project_dir / 'branch_compare_v3_2.json'
        self.branch_verdict_md_file = self.project_dir / 'branch_verdict_v3_2.md'
        self.branch_verdict_txt_file = self.project_dir / 'branch_verdict_v3_2.txt'
        self.runtime_dashboard_json = self.project_dir / 'runtime_dashboard_v3_2.json'
        self.runtime_dashboard_txt = self.project_dir / 'runtime_dashboard_v3_2.txt'
        self.runtime_dashboard_md = self.project_dir / 'runtime_dashboard_v3_2.md'
        self.archive_summary_file = self.project_dir / 'archive_summary.json'
        self.release_summary_json = self.project_dir / 'release_summary_v3_2.json'
        self.release_summary_txt = self.project_dir / 'release_summary_v3_2.txt'
        self.report_txt = self.project_dir / 'report_v3.2.txt'
        self.report_md = self.project_dir / 'report_v3.2.md'
        self.session_query_json = self.project_dir / 'session_query_v3_2.json'
        self.session_query_txt = self.project_dir / 'session_query_v3_2.txt'
        self.timeline_json = self.project_dir / 'history_timeline_v3_2.json'
        self.timeline_txt = self.project_dir / 'history_timeline_v3_2.txt'
        self.timeline_md = self.project_dir / 'history_timeline_v3_2.md'
    ...
    def build_compact_handoff(self):
        ...
        data = {
            ...
            'resume_main': f"python morph_runtime_core_v3_2.py --append-task math --resume-session {latest['session_id']}" if latest else None,
            'resume_simple': "python morph_runtime_core_v3_2.py --append-task simple --branch simple_track" if latest else None,
        }
    ...
    def build_quick_resume_pack(self):
        ...
        data = {
            ...
            'main_resume': f"python morph_runtime_core_v3_2.py --append-task math --resume-session {latest['session_id']}" if latest and latest['task_type'] == 'math' else None,
            'simple_resume': "python morph_runtime_core_v3_2.py --append-task simple --branch simple_track",
            'checkpoint_create': "python morph_runtime_core_v3_2.py --checkpoint-label stable_v3_2",
    ...
    def branch_verdicts(self):
        ...
        md_lines = ['# Branch Verdicts v3.2', '']
        txt_lines = ['Branch Verdicts v3.2', '']
    ...
    def build_release_summary(self):
        ...
        lines = [
            ...
            f'Dashboard MD: {self.runtime_dashboard_md.name}',
            f'Dashboard JSON: {self.runtime_dashboard_json.name}',
            f'Query TXT: {self.session_query_txt.name}',
            f'Query JSON: {self.session_query_json.name}',
            f'Timeline TXT: {self.timeline_txt.name}',
            f'Timeline JSON: {self.timeline_json.name}',
        ]
    ...
    def build_timeline(self):
        ...
        for a in index.get('audit_tail', []):
            entries.append({
                ...
                'branch_label': a.get('branch_label'),
        ...
        for e in entries:
            if e['type'] == 'session':
                lines.append(
                    f"- SESSION | {e.get('timestamp')} | {e.get('session_id')} | parent={e.get('parent')} | "
                    f"branch={e.get('branch')} | task={e.get('task_type')} | route={e.get('chosen_route')} | "
                    f"health={e.get('health_score')} | deferred={e.get('deferred_item_count')}"
                )
            else:
                lines.append(
                    f"- AUDIT | {e.get('timestamp')} | event={e.get('event_type')} | session={e.get('session_id')} | "
                    f"branch={e.get('branch_label')} | route={e.get('route_name')} | outcome={e.get('outcome')}"
                )
        write_text(self.timeline_txt, '\n'.join(lines) + '\n')
        md = ['# History Timeline v3.2', '', f'- **Project:** {self.project_id}', f'- **Entry count:** `{len(entries)}`', '', '## Entries']
        for e in entries:
            if e['type'] == 'session':
                md.append(
                    f"- `SESSION` | `{e.get('timestamp')}` | `{e.get('session_id')}` | parent=`{e.get('parent')}` | "
    ...
    def build_upload_reports(self):
        ...
        txt_lines = [
            ...
            'V3.2 features:',
            '- history timeline',
            '- ordered session + audit history',
            '- timeline export txt / json / md',
        ]
        write_text(self.report_txt, '\n'.join(txt_lines) + '\n')
        write_text(self.report_md, '# Report v3.2\n\n' + '\n'.join(f'- {line}' for line in txt_lines if line.strip()) + '\n')
    ...
    def print_summary(self, session, root_created):
        print('=== MORPH Runtime Core v3.2 / Summary ===')
        ...
        print(f'Timeline txt:   {self.timeline_txt}')
        print(f'Timeline md:    {self.timeline_md}')
        print(f'Timeline json:  {self.timeline_json}')
    ...
    def cli(self):
        parser = argparse.ArgumentParser(description='MORPH Runtime Core v3.2')
...
def main():
    MorphRuntimeCoreV32(Path.cwd()).cli()
