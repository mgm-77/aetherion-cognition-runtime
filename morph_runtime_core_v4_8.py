...
VERSION = "v4.8"
PACK_ROOT_NAME = "morph_runtime_core_v4_8"
...
class MorphRuntimeCoreV48:
    def __init__(self, base_dir: Path):
        ...
        self.report_txt = self.project_dir / "report_v4.8.txt"
        self.report_md = self.project_dir / "report_v4.8.md"
        self.historical_txt = self.project_dir / "historical_target_v4_8.txt"
        self.historical_json = self.project_dir / "historical_target_v4_8.json"
        self.current_txt = self.project_dir / "current_target_v4_8.txt"
        self.current_json = self.project_dir / "current_target_v4_8.json"
        self.recent_txt = self.project_dir / "recent_target_v4_8.txt"
        self.recent_json = self.project_dir / "recent_target_v4_8.json"
        self.conflict_txt = self.project_dir / "target_conflict_v4_8.txt"
        self.conflict_json = self.project_dir / "target_conflict_v4_8.json"
        self.switch_txt = self.project_dir / "target_switch_v4_8.txt"
        self.switch_json = self.project_dir / "target_switch_v4_8.json"
        self.switch_md = self.project_dir / "target_switch_v4_8.md"
        self.lock_txt = self.project_dir / "target_lock_v4_8.txt"
        self.lock_json = self.project_dir / "target_lock_v4_8.json"
        self.protection_txt = self.project_dir / "target_protection_v4_8.txt"
        self.protection_json = self.project_dir / "target_protection_v4_8.json"
        self.drift_alert_txt = self.project_dir / "target_drift_alert_v4_8.txt"
        self.drift_alert_json = self.project_dir / "target_drift_alert_v4_8.json"

        self.recovery_route_txt = self.project_dir / "recovery_route_v4_8.txt"
        self.recovery_route_json = self.project_dir / "recovery_route_v4_8.json"
        self.recovery_route_md = self.project_dir / "recovery_route_v4_8.md"
        self.auto_return_txt = self.project_dir / "auto_return_v4_8.txt"
        self.auto_return_json = self.project_dir / "auto_return_v4_8.json"
        self.auto_return_md = self.project_dir / "auto_return_v4_8.md"
        self.safe_resume_txt = self.project_dir / "safe_resume_path_v4_8.txt"
        self.safe_resume_json = self.project_dir / "safe_resume_path_v4_8.json"
        self.safe_resume_md = self.project_dir / "safe_resume_path_v4_8.md"
    ...
    def build_switch_recommendation(self):
        ...
        write_text(self.switch_md, "# Target Switch v4.8\n\n" + "\n".join(f"- {x}" for x in lines) + "\n")
        return payload
    ...
    def build_recovery_route(self):
        ...
        if session_id:
            ...
            branch = s.get("branch")
            if branch == "main":
                command = f"python morph_runtime_core_v4_8.py --append-task {task} --resume-session {session_id}"
            else:
                command = f"python morph_runtime_core_v4_8.py --append-task {task} --resume-session {session_id} --branch {branch}"
        ...
        write_text(self.recovery_route_md, "# Recovery Route v4.8\n\n" + "\n".join(f"- {x}" for x in lines) + "\n")
        return payload
    ...
    def build_auto_return(self):
        ...
        write_text(self.auto_return_md, "# Auto Return v4.8\n\n" + "\n".join(f"- {x}" for x in lines) + "\n")
        return payload
    ...
    def build_safe_resume_path(self):
        ...
        write_text(self.safe_resume_md, "# Safe Resume Path v4.8\n\n" + "\n".join(f"- {x}" for x in lines) + "\n")
        return payload
    ...
    def build_upload_reports(self):
        ...
        txt_lines = [
            ...
            "- auto-return recommendation",
            "- best safe resume path",
            "- safe recovery after drift",
        ]
        write_text(self.report_txt, "\n".join(txt_lines) + "\n")
        write_text(self.report_md, "# Report v4.8\n\n" + "\n".join(f"- {x}" for x in txt_lines if x.strip()) + "\n")
    ...
    def print_summary(self, session, root_created):
        print("=== MORPH Runtime Core v4.8 / Summary ===")
        ...
        print(f"Recovery route txt: {self.recovery_route_txt}")
        print(f"Auto return txt:    {self.auto_return_txt}")
        print(f"Safe resume txt:    {self.safe_resume_txt}")
        print(f"ZIP export:         {self.zip_export_file}")

    def cli(self):
        parser = argparse.ArgumentParser(description="MORPH Runtime Core v4.8")
        ...
        parser.add_argument("--recovery-route-only", action="store_true")
        parser.add_argument("--auto-return-only", action="store_true")
        parser.add_argument("--safe-resume-path-only", action="store_true")
        ...
        if args.report_only:
            ...
            print(self.recovery_route_txt)
            print(self.auto_return_txt)
            print(self.safe_resume_txt)
            return

        if args.recovery_route_only:
            self.init_pack()
            self.build_all_reports()
            print(self.recovery_route_txt)
            print(self.recovery_route_json)
            return

        if args.auto_return_only:
            self.init_pack()
            self.build_all_reports()
            print(self.auto_return_txt)
            print(self.auto_return_json)
            return

        if args.safe_resume_path_only:
            self.init_pack()
            self.build_all_reports()
            print(self.safe_resume_txt)
            print(self.safe_resume_json)
            return
...
def main():
    MorphRuntimeCoreV48(Path.cwd()).cli()
