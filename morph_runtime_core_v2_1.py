...
VERSION = 'v2.1'
PACK_ROOT_NAME = 'morph_runtime_core_v2_1'
...
MODULES_BY_TASK = {
    'math': [
        ...
        'FabricOrchestratorV21','BranchScoringLayer','ConflictResolver','QueueArbitrationLayer','MultiScaleScheduler',
        ...
        'ReplayEngine','CandidateRouteGenerator','HorizonPlanner','EarlyCollapseController','MergeResolver','ReportBuilder'
    ],
    'simple': [
        'RuntimeKernel','RuntimeStateManager','RuntimeIndexManager','ContinuityPackManager','SessionStateAdapter',
        'ComparativeResumeEngine','HandoffExporter','PackValidator','IntegrityGuard','RecoveryEngine',
        'FabricOrchestratorV21','PromptClassifier','AuditTrailEngine','ReportBuilder'
    ]
}
TASK_DEFAULTS = {
    'math': {
...
def summarize(index: Dict[str, Any]) -> Dict[str, Any]:
    ...
    out = {
        ...
        'resume_main': f'python morph_runtime_core_v2_1.py --resume-session session_002' if 'session_002' in sessions else None,
        'resume_simple': None
    }
    for sid in ids:
        if sessions[sid]['branch'] != 'main':
            out['resume_simple'] = f'python morph_runtime_core_v2_1.py --resume-session {sid}'
            break
    return out
...
def build_txt_report(summary: Dict[str, Any], index: Dict[str, Any], project_dir: Path) -> str:
    ...
    lines = [
        ...
        f"- Health: {latest.get('health_score')}",'',
        'Generated Files','- runtime_index.json','- project_manifest.json','- integrity_report.json','- compact_handoff.json',
        '- quick_report.txt',f'- report_{VERSION}.txt',f'- report_{VERSION}.md','',
        'Sessions'
    ]
    for sid in ids:
    ...
    lines += ['', 'Executive Summary', 'The runtime pack is healthy, resumable, and ready for upload or review.',
              f"Main report path: {project_dir / ('report_' + VERSION + '.txt')}",
              f"Markdown report path: {project_dir / ('report_' + VERSION + '.md')}"]
    return '\n'.join(lines) + '\n'
...
def build_md_report(summary: Dict[str, Any], index: Dict[str, Any]) -> str:
    ...
    for sid in ids:
        s = index['sessions'][sid]
        lines.append(f"- `{sid}` — branch=`{s['branch']}` | task=`{s['task_type']}` | route=`{s['chosen_route']}` | saved_at=`{s['saved_at']}`")
    lines += ['', '## Generated Files', '- `runtime_index.json`','- `project_manifest.json`','- `integrity_report.json`',
              '- `compact_handoff.json`','- `quick_report.txt`',f'- `report_{VERSION}.txt`',f'- `report_{VERSION}.md`','', '## Audit Tail']
    for item in index['audit_tail'][-8:]:
        lines.append(f"- `{item['event_type']}` | `{item['outcome']}` | `{item.get('session_id')}`")
    lines += ['', '## Executive Summary', 'This project pack is resumable, structured, and ready to be uploaded directly.']
    return '\n'.join(lines) + '\n'
...
def save_outputs(project_dir: Path, index: Dict[str, Any]) -> Dict[str, Path]:
    ...
    return {'runtime_index': runtime_index,'manifest': manifest,'integrity': integrity,'handoff': handoff,
            'quick': quick,'report_txt': report_txt,'report_md': report_md,'zip': Path(zip_path)}
...
def main():
    ...
    print(f"ZIP export:      {outputs['zip']}")
    print('\n=== Status Snapshot ===')
    print(json.dumps({
