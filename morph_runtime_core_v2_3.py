...
VERSION = 'v2.3'
PACK_ROOT_NAME = 'morph_runtime_core_v2_3'
...
MODULES_BY_TASK = {
    'math': [
        ...
        'FabricOrchestratorV23','BranchScoringLayer','ConflictResolver','QueueArbitrationLayer','MultiScaleScheduler',
        ...
        'ReportBuilder','FinalUploadBundler','UploadOnlyRebuilder'
    ],
    'simple': [
        'RuntimeKernel','RuntimeStateManager','RuntimeIndexManager','ContinuityPackManager','SessionStateAdapter',
        'ComparativeResumeEngine','HandoffExporter','PackValidator','IntegrityGuard','RecoveryEngine',
        'FabricOrchestratorV23','PromptClassifier','AuditTrailEngine','ReportBuilder','FinalUploadBundler',
        'UploadOnlyRebuilder'
    ]
}
...
def summarize(index: Dict[str, Any]) -> Dict[str, Any]:
    ...
    out = {
        ...
        'resume_main': None,'resume_simple': None
    }
    main_ids = [sid for sid in ids if sessions[sid]['branch'] == 'main']
    if main_ids:
        out['resume_main'] = f'python morph_runtime_core_v2_3.py --resume-session {main_ids[-1]} --append-task math'
    for sid in ids:
        if sessions[sid]['branch'] != 'main':
            out['resume_simple'] = f'python morph_runtime_core_v2_3.py --resume-session {sid} --append-task simple --branch {sessions[sid]["branch"]}'
            break
    return out
...
def build_quick_report(summary: Dict[str, Any]) -> str:
    ...
    lines = [
        ...
        'Rebuild upload only: python morph_runtime_core_v2_3.py --build-upload-only'
    ]
    return '\n'.join(lines) + '\n'
...
def build_txt_report(summary: Dict[str, Any], index: Dict[str, Any], project_dir: Path) -> str:
    ...
    lines += [
        '', 'Upload Ready',
        '- final_upload_bundle/report_v2.3.txt',
        '- final_upload_bundle/report_v2.3.md',
        ...
        '',
        'Rebuild Command',
        'python morph_runtime_core_v2_3.py --build-upload-only',
        '',
        'Executive Summary',
...
def build_upload_bundle(project_dir: Path, outputs: Dict[str, Path], summary: Dict[str, Any]) -> Dict[str, Path]:
    ...
    selected = {
        'report_v2.3.txt': outputs['report_txt'],
        'report_v2.3.md': outputs['report_md'],
...
def main():
    ...
    if args.build_upload_only:
        if not runtime_index.exists():
            raise SystemExit('No existing project pack found for --build-upload-only.')
        index = read_json(runtime_index)
        append_audit(index, 'rebuild_upload_only', 'rebuilt', None, {'path': 'final_upload_bundle/'})
        outputs = save_outputs(project_dir, index)
        summary = summarize(index)
        latest = summary.get('latest_session') or {}
        print(f'=== MORPH Runtime Core {VERSION} / Upload Rebuild ===')
        ...
        print('\n=== Status Snapshot ===')
        print(json.dumps({
            'session_id': latest.get('session_id'),'branch': latest.get('branch'),
    ...
    if not runtime_index.exists():
        if not args.fresh_if_missing and not args.append_task:
            raise SystemExit('No project pack found. Use --fresh-if-missing, --append-task, or --build-upload-only.')
