...
VERSION = 'v3.9'
PACK_ROOT_NAME = 'morph_runtime_core_v3_9'
...
MODULES_BY_TASK = {
    'math': [
        ...
        'QuickResumePack','SummaryPackBuilder','PackValidator','IntegrityGuard','FabricOrchestratorV39',
        ...
        'RegressionDetector','AnomalyDetector','TrendDetector'
    ],
    'simple': [
        'RuntimeKernel','RuntimeStateManager','RuntimeIndexManager','ContinuityPackManager','SessionStateAdapter',
        'RecoveryEngine','AutoRepairBasic','OrphanHandler','ComparativeResumeEngine','HandoffExporter',
...
class MorphRuntimeCoreV39:
    def __init__(self, base_dir: Path):
        ...
        self.branch_compare_file = self.project_dir / 'branch_compare_v3_9.json'
        self.branch_verdict_md_file = self.project_dir / 'branch_verdict_v3_9.md'
        self.branch_verdict_txt_file = self.project_dir / 'branch_verdict_v3_9.txt'
        self.runtime_dashboard_json = self.project_dir / 'runtime_dashboard_v3_9.json'
        self.runtime_dashboard_txt = self.project_dir / 'runtime_dashboard_v3_9.txt'
        self.runtime_dashboard_md = self.project_dir / 'runtime_dashboard_v3_9.md'
        self.archive_summary_file = self.project_dir / 'archive_summary.json'
        self.release_summary_json = self.project_dir / 'release_summary_v3_9.json'
        self.release_summary_txt = self.project_dir / 'release_summary_v3_9.txt'
        self.report_txt = self.project_dir / 'report_v3.9.txt'
        self.report_md = self.project_dir / 'report_v3.9.md'
        self.session_query_json = self.project_dir / 'session_query_v3_9.json'
        self.session_query_txt = self.project_dir / 'session_query_v3_9.txt'
        self.timeline_json = self.project_dir / 'history_timeline_v3_9.json'
        self.timeline_txt = self.project_dir / 'history_timeline_v3_9.txt'
        self.timeline_md = self.project_dir / 'history_timeline_v3_9.md'
        self.pressure_json = self.project_dir / 'pressure_signals_v3_9.json'
        self.pressure_txt = self.project_dir / 'pressure_signals_v3_9.txt'
        self.pressure_md = self.project_dir / 'pressure_signals_v3_9.md'
        self.drift_json = self.project_dir / 'drift_notes_v3_9.json'
        self.drift_txt = self.project_dir / 'drift_notes_v3_9.txt'
        self.drift_md = self.project_dir / 'drift_notes_v3_9.md'
        self.checkpoint_diff_json = self.project_dir / 'checkpoint_diff_v3_9.json'
        self.checkpoint_diff_txt = self.project_dir / 'checkpoint_diff_v3_9.txt'
        self.checkpoint_diff_md = self.project_dir / 'checkpoint_diff_v3_9.md'
        self.checkpoint_registry_json = self.project_dir / 'checkpoint_registry_v3_9.json'
        self.checkpoint_registry_txt = self.project_dir / 'checkpoint_registry_v3_9.txt'
        self.checkpoint_registry_md = self.project_dir / 'checkpoint_registry_v3_9.md'
        self.checkpoint_lineage_json = self.project_dir / 'checkpoint_lineage_v3_9.json'
        self.checkpoint_lineage_txt = self.project_dir / 'checkpoint_lineage_v3_9.txt'
        self.checkpoint_lineage_md = self.project_dir / 'checkpoint_lineage_v3_9.md'
        self.checkpoint_chain_json = self.project_dir / 'checkpoint_chain_v3_9.json'
        self.checkpoint_chain_txt = self.project_dir / 'checkpoint_chain_v3_9.txt'
        self.checkpoint_chain_md = self.project_dir / 'checkpoint_chain_v3_9.md'
        self.best_checkpoint_json = self.project_dir / 'best_checkpoint_v3_9.json'
        self.best_checkpoint_txt = self.project_dir / 'best_checkpoint_v3_9.txt'
        self.best_checkpoint_md = self.project_dir / 'best_checkpoint_v3_9.md'
        self.leaderboard_json = self.project_dir / 'checkpoint_leaderboard_v3_9.json'
        self.leaderboard_txt = self.project_dir / 'checkpoint_leaderboard_v3_9.txt'
        self.leaderboard_md = self.project_dir / 'checkpoint_leaderboard_v3_9.md'
        self.regression_json = self.project_dir / 'regression_report_v3_9.json'
        self.regression_txt = self.project_dir / 'regression_report_v3_9.txt'
        self.regression_md = self.project_dir / 'regression_report_v3_9.md'
        self.anomaly_json = self.project_dir / 'anomaly_report_v3_9.json'
        self.anomaly_txt = self.project_dir / 'anomaly_report_v3_9.txt'
        self.anomaly_md = self.project_dir / 'anomaly_report_v3_9.md'
        self.trend_json = self.project_dir / 'trend_report_v3_9.json'
        self.trend_txt = self.project_dir / 'trend_report_v3_9.txt'
        self.trend_md = self.project_dir / 'trend_report_v3_9.md'
    ...
    def build_compact_handoff(self):
        ...
        data = {
            ...
            'resume_main': f"python morph_runtime_core_v3_9.py --append-task math --resume-session {latest['session_id']}" if latest else None,
            'resume_simple': "python morph_runtime_core_v3_9.py --append-task simple --branch simple_track" if latest else None,
        }
    ...
    def build_quick_resume_pack(self):
        ...
        data = {
            ...
            'main_resume': f"python morph_runtime_core_v3_9.py --append-task math --resume-session {latest['session_id']}" if latest and latest['task_type'] == 'math' else None,
            'simple_resume': "python morph_runtime_core_v3_9.py --append-task simple --branch simple_track",
            'checkpoint_create': "python morph_runtime_core_v3_9.py --checkpoint-label stable_v3_9",
    ...
    def branch_verdicts(self):
        ...
        md_lines = ['# Branch Verdicts v3.9', '']
        txt_lines = ['Branch Verdicts v3.9', '']
    ...
    def build_timeline(self):
        ...
        md = ['# History Timeline v3.9', '', f'- **Project:** {self.project_id}', f'- **Entry count:** `{len(entries)}`', '', '## Entries']
        for e in entries:
            if e['type'] == 'session':
    ...
    def build_pressure_signals(self):
        ...
        for sid in sorted(index['sessions'].keys()):
            ...
            signals.append({
                ...
                'chosen_route': s.get('chosen_route'),'route_confidence': s.get('route_confidence'),
                'health_score': s.get('health_score'),'deferred_item_count': s.get('deferred_item_count'),
                'pressure_score': pressure_score,'pressure_level': level,'saved_at': s.get('saved_at'),
            })
        ...
        md = ['# Pressure Signals v3.9', '', f'- **Project:** {self.project_id}', f'- **Signal count:** `{len(signals)}`', '', '## Signals']
        for s in signals:
    ...
    def build_drift_notes(self):
        ...
        for branch, info in index['branches'].items():
            ...
            note = {
                ...
                'session_count': len(sids),'route_start': first.get('chosen_route'),'route_end': last.get('chosen_route'),
        ...
        md = ['# Drift Notes v3.9', '', f'- **Project:** {self.project_id}', f'- **Branch count:** `{len(notes)}`', '', '## Notes']
        for n in notes:
    ...
    def build_checkpoint_registry(self):
        ...
        md = ['# Checkpoint Registry v3.9', '', f'- **Project:** {self.project_id}', f'- **Checkpoint count:** `{len(entries)}`', '', '## Checkpoints']
        for e in entries:
    ...
    def build_checkpoint_lineage(self):
        ...
        md = ['# Checkpoint Lineage v3.9', '', f'- **Project:** {self.project_id}', f'- **Lineage count:** `{len(lineage)}`', '', '## Lineage']
        for e in lineage:
    ...
    def build_checkpoint_chain(self):
        ...
        md = ['# Checkpoint Chain v3.9', '', f'- **Project:** {self.project_id}', f'- **Link count:** `{len(links)}`', '', '## Links']
        for l in links:
    ...
    def build_leaderboard(self):
        ...
        md = ['# Checkpoint Leaderboard v3.9', '', f'- **Project:** {self.project_id}', f'- **Checkpoint count:** `{len(leaderboard)}`', '', '## Leaderboard']
        for i, e in enumerate(leaderboard, start=1):
            md.append(f"{i}. `{e['label']}` — score=`{e['score']}` | branch=`{e['branch']}` | task=`{e['task_type']}` | route=`{e['route']}` | conf=`{e['confidence']}` | health=`{e['health']}` | deferred=`{e['deferred']}`")
        md.extend(['', '## Best per branch'])
        for k, v in best_per_branch.items():
            md.append(f"- `{k}`: `{v}`")
        md.extend(['', '## Best per task'])
        for k, v in best_per_task.items():
            md.append(f"- `{k}`: `{v}`")
        write_text(self.leaderboard_md, '\n'.join(md) + '\n')
        return payload
    ...
    def build_regression_report(self):
        ...
        md = ['# Regression Report v3.9', '', f'- **Project:** {self.project_id}', f'- **Branch count:** `{len(findings)}`', '', '## Findings']
        for f in findings:
            if f.get('verdict') == 'insufficient_history':
                md.append(f"- `{f['branch']}` — verdict=`insufficient_history`")
            else:
                md.append(f"- `{f['branch']}` — start=`{f['start_session_id']}` | end=`{f['end_session_id']}` | confidence_delta=`{f['confidence_delta']}` | health_delta=`{f['health_delta']}` | deferred_delta=`{f['deferred_delta']}` | verdict=`{f['verdict']}`")
        write_text(self.regression_md, '\n'.join(md) + '\n')
        return payload
    ...
    def build_anomaly_report(self):
        ...
        md = ['# Anomaly Report v3.9', '', f'- **Project:** {self.project_id}', f'- **Anomaly count:** `{len(anomalies)}`', '', '## Anomalies']
        for a in anomalies:
            md.append(f"- `{a['session_id']}` — branch=`{a['branch']}` | task=`{a['task_type']}` | route=`{a['route']}` | health=`{a['health']}` | confidence=`{a['confidence']}` | deferred=`{a['deferred']}` | reasons=`{','.join(a['reasons'])}`")
        write_text(self.anomaly_md, '\n'.join(md) + '\n')
        return payload
    ...
    def build_trend_report(self):
        ...
        md = ['# Trend Report v3.9', '', f'- **Project:** {self.project_id}', f'- **Branch count:** `{len(trends)}`', '', '## Trends']
        for t in trends:
            if t.get('reason') == 'insufficient_history':
                md.append(f"- `{t['branch']}` — trend=`flat` | reason=`insufficient_history`")
            else:
                md.append(f"- `{t['branch']}` — start=`{t['start_session_id']}` | end=`{t['end_session_id']}` | confidence_delta=`{t['confidence_delta']}` | health_delta=`{t['health_delta']}` | deferred_delta=`{t['deferred_delta']}` | trend=`{t['trend']}`")
        write_text(self.trend_md, '\n'.join(md) + '\n')
        return payload
    ...
    def build_release_summary(self):
        ...
        data = {
            ...
            'files': {
                ...
                'leaderboard_txt': str(self.leaderboard_txt),
                'best_checkpoint_txt': str(self.best_checkpoint_txt),
                'regression_txt': str(self.regression_txt),
                'anomaly_txt': str(self.anomaly_txt),
                'trend_txt': str(self.trend_txt),
            }
        }
        ...
        lines = [
            ...
            f'Regression TXT: {self.regression_txt.name}',
            f'Anomaly TXT: {self.anomaly_txt.name}',
            f'Trend TXT: {self.trend_txt.name}',
        ]
    ...
    def print_summary(self, session, root_created):
        print('=== MORPH Runtime Core v3.9 / Summary ===')
        ...
        print(f'Regression txt: {self.regression_txt}')
        print(f'Anomaly txt:    {self.anomaly_txt}')
        print(f'Trend txt:      {self.trend_txt}')
    ...
    def cli(self):
        parser = argparse.ArgumentParser(description='MORPH Runtime Core v3.9')
        ...
        if args.leaderboard_only:
            self.init_pack()
            payload = self.build_leaderboard()
            print(json.dumps(payload, indent=2, ensure_ascii=False))
            print(self.leaderboard_txt)
            print(self.leaderboard_json)
            return
...
def main():
    MorphRuntimeCoreV39(Path.cwd()).cli()
