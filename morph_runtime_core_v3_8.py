...
VERSION = 'v3.8'
PACK_ROOT_NAME = 'morph_runtime_core_v3_8'
...
MODULES_BY_TASK = {
    'math': [
        ...
        'QuickResumePack','SummaryPackBuilder','PackValidator','IntegrityGuard','FabricOrchestratorV38',
...
class MorphRuntimeCoreV38:
    def __init__(self, base_dir: Path):
        ...
        self.branch_compare_file = self.project_dir / 'branch_compare_v3_8.json'
        self.branch_verdict_md_file = self.project_dir / 'branch_verdict_v3_8.md'
        self.branch_verdict_txt_file = self.project_dir / 'branch_verdict_v3_8.txt'
        self.runtime_dashboard_json = self.project_dir / 'runtime_dashboard_v3_8.json'
        self.runtime_dashboard_txt = self.project_dir / 'runtime_dashboard_v3_8.txt'
        self.runtime_dashboard_md = self.project_dir / 'runtime_dashboard_v3_8.md'
        self.archive_summary_file = self.project_dir / 'archive_summary.json'
        self.release_summary_json = self.project_dir / 'release_summary_v3_8.json'
        self.release_summary_txt = self.project_dir / 'release_summary_v3_8.txt'
        self.report_txt = self.project_dir / 'report_v3.8.txt'
        self.report_md = self.project_dir / 'report_v3.8.md'
        self.session_query_json = self.project_dir / 'session_query_v3_8.json'
        self.session_query_txt = self.project_dir / 'session_query_v3_8.txt'
        self.timeline_json = self.project_dir / 'history_timeline_v3_8.json'
        self.timeline_txt = self.project_dir / 'history_timeline_v3_8.txt'
        self.timeline_md = self.project_dir / 'history_timeline_v3_8.md'
        self.pressure_json = self.project_dir / 'pressure_signals_v3_8.json'
        self.pressure_txt = self.project_dir / 'pressure_signals_v3_8.txt'
        self.pressure_md = self.project_dir / 'pressure_signals_v3_8.md'
        self.drift_json = self.project_dir / 'drift_notes_v3_8.json'
        self.drift_txt = self.project_dir / 'drift_notes_v3_8.txt'
        self.drift_md = self.project_dir / 'drift_notes_v3_8.md'
        self.checkpoint_diff_json = self.project_dir / 'checkpoint_diff_v3_8.json'
        self.checkpoint_diff_txt = self.project_dir / 'checkpoint_diff_v3_8.txt'
        self.checkpoint_diff_md = self.project_dir / 'checkpoint_diff_v3_8.md'
        self.checkpoint_registry_json = self.project_dir / 'checkpoint_registry_v3_8.json'
        self.checkpoint_registry_txt = self.project_dir / 'checkpoint_registry_v3_8.txt'
        self.checkpoint_registry_md = self.project_dir / 'checkpoint_registry_v3_8.md'
        self.checkpoint_lineage_json = self.project_dir / 'checkpoint_lineage_v3_8.json'
        self.checkpoint_lineage_txt = self.project_dir / 'checkpoint_lineage_v3_8.txt'
        self.checkpoint_lineage_md = self.project_dir / 'checkpoint_lineage_v3_8.md'
        self.checkpoint_chain_json = self.project_dir / 'checkpoint_chain_v3_8.json'
        self.checkpoint_chain_txt = self.project_dir / 'checkpoint_chain_v3_8.txt'
        self.checkpoint_chain_md = self.project_dir / 'checkpoint_chain_v3_8.md'
        self.best_checkpoint_json = self.project_dir / 'best_checkpoint_v3_8.json'
        self.best_checkpoint_txt = self.project_dir / 'best_checkpoint_v3_8.txt'
        self.best_checkpoint_md = self.project_dir / 'best_checkpoint_v3_8.md'
        self.leaderboard_json = self.project_dir / 'checkpoint_leaderboard_v3_8.json'
        self.leaderboard_txt = self.project_dir / 'checkpoint_leaderboard_v3_8.txt'
        self.leaderboard_md = self.project_dir / 'checkpoint_leaderboard_v3_8.md'
    ...
    def build_compact_handoff(self):
        ...
        data = {
            ...
            'resume_main': f"python morph_runtime_core_v3_8.py --append-task math --resume-session {latest['session_id']}" if latest else None,
            'resume_simple': "python morph_runtime_core_v3_8.py --append-task simple --branch simple_track" if latest else None,
        }
    ...
    def build_quick_resume_pack(self):
        ...
        data = {
            ...
            'main_resume': f"python morph_runtime_core_v3_8.py --append-task math --resume-session {latest['session_id']}" if latest and latest['task_type'] == 'math' else None,
            'simple_resume': "python morph_runtime_core_v3_8.py --append-task simple --branch simple_track",
            'checkpoint_create': "python morph_runtime_core_v3_8.py --checkpoint-label stable_v3_8",
    ...
    def branch_verdicts(self):
        ...
        md_lines = ['# Branch Verdicts v3.8', '']
        txt_lines = ['Branch Verdicts v3.8', '']
    ...
    def build_timeline(self):
        ...
        md = ['# History Timeline v3.8', '', f'- **Project:** {self.project_id}', f'- **Entry count:** `{len(entries)}`', '', '## Entries']
        for e in entries:
            if e['type'] == 'session':
    ...
    def build_pressure_signals(self):
        ...
        md = ['# Pressure Signals v3.8', '', f'- **Project:** {self.project_id}', f'- **Signal count:** `{len(signals)}`', '', '## Signals']
        for s in signals:
    ...
    def build_drift_notes(self):
        ...
        md = ['# Drift Notes v3.8', '', f'- **Project:** {self.project_id}', f'- **Branch count:** `{len(notes)}`', '', '## Notes']
        for n in notes:
    ...
    def build_checkpoint_registry(self):
        ...
        md = ['# Checkpoint Registry v3.8', '', f'- **Project:** {self.project_id}', f'- **Checkpoint count:** `{len(entries)}`', '', '## Checkpoints']
        for e in entries:
    ...
    def build_checkpoint_lineage(self):
        ...
        md = ['# Checkpoint Lineage v3.8', '', f'- **Project:** {self.project_id}', f'- **Lineage count:** `{len(lineage)}`', '', '## Lineage']
        for e in lineage:
    ...
    def build_checkpoint_chain(self):
        ...
        md = ['# Checkpoint Chain v3.8', '', f'- **Project:** {self.project_id}', f'- **Link count:** `{len(links)}`', '', '## Links']
        for l in links:
    ...
    def _checkpoint_score(self, entry):
        ...
        score = round((health * 100) + (conf * 50) - (deferred * 10), 3)
        return score
    ...
    def build_best_checkpoint(self):
        ...
        if entries:
            ...
            for e in entries:
                x = dict(e)
                x['score'] = self._checkpoint_score(x)
                enriched.append(x)
    ...
    def build_leaderboard(self):
        ...
        md = ['# Checkpoint Leaderboard v3.8', '', f'- **Project:** {self.project_id}', f'- **Checkpoint count:** `{len(leaderboard)}`', '', '## Leaderboard']
        for i, e in enumerate(leaderboard, start=1):
    ...
    def build_release_summary(self):
        ...
        lines = [
            ...
            f'Leaderboard TXT: {self.leaderboard_txt.name}',
    ...
    def print_summary(self, session, root_created):
        print('=== MORPH Runtime Core v3.8 / Summary ===')
        ...
        print(f'Leaderboard txt:{self.leaderboard_txt}')
    ...
    def cli(self):
        parser = argparse.ArgumentParser(description='MORPH Runtime Core v3.8')
...
def main():
    MorphRuntimeCoreV38(Path.cwd()).cli()
