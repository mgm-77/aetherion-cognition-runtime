"""
MORPH Runtime Core v1.1
Multi-Session Continuity Pack

Adds on top of v1.0:
- ContinuityPackManager
- project_id / session_id
- session registry
- project-scoped saves
- selective session resume
- project listing
- session listing
...
class PromptClassifier:
    ...
    def classify(cls, prompt: str, context_items: List[ContextItem]) -> TaskType:
        ...
        scores = {
            TaskType.SIMPLE: sum(w for h, w in {"summary":1.4,"short":1,"friendly":1,"brief":1}.items() if h in lower),
            TaskType.RETRIEVAL: sum(w for h, w in {"find":1.7,"search":1.7,"extract":1.7,"retrieve":1.7}.items() if h in lower),
            TaskType.CODING: sum(w for h, w in {"code":2,"python":2,"javascript":2,"bug":1.8,"function":1.7,"debug":2}.items() if h in lower),
            TaskType.MATH: sum(w for h, w in {"prove":2,"lemma":1.8,"theorem":1.8,"equation":1.6,"conjecture":2,"contradiction":1.7}.items() if h in lower),
            TaskType.VERIFICATION_HEAVY: sum(w for h, w in {"verify":2,"validate":2,"audit":2,"prove rigorously":2.2}.items() if h in lower),
            TaskType.LONG_CONTEXT: 2.2 if "long context" in lower else 0.0,
        }
...
class TokenDifficultyEstimator:
    ...
    def estimate(cls, chunk: str, task_type: TaskType) -> Difficulty:
        lower = chunk.lower()
        if any(x in lower for x in ["prove", "contradiction", "verify", "validate", "="]):
...
class VerificationTrigger:
    @staticmethod
    def should_verify(task_type: TaskType, difficulty: Difficulty, chosen_depth: int, budget: BudgetProfile, chunk: str, historical_record: Optional[RouteMemoryRecord] = None, chosen_route: Optional[CandidateRoute] = None) -> bool:
...
class DepthGovernor:
    ...
    @classmethod
    def choose_depth(cls, difficulty: Difficulty, task_type: TaskType, budget: BudgetProfile, historical_record: Optional[RouteMemoryRecord] = None, chosen_route: Optional[CandidateRoute] = None, horizon: Optional[Horizon] = None) -> int:
...
class ContinuityPackManager:
    def __init__(self, root_dir: str = "morph_continuity_pack_v1_1") -> None:
    ...
    def save_session(self, project_id: str, session_id: str, payload: Dict) -> Path:
        file_path = self.get_session_file(project_id, session_id)
        file_path.write_text(json.dumps(payload, indent=2), encoding="utf-8")
        registry = self._load_registry()
        registry["projects"].setdefault(project_id, {"sessions": []})
    ...
    def list_sessions(self, project_id: str) -> List[str]:
        registry = self._load_registry()
        return sorted(registry["projects"].get(project_id, {}).get("sessions", []))


class SessionStateAdapter:
    @staticmethod
    def export_payload(project_id: str, session_id: str, budget: BudgetProfile, route_memory: RouteMemoryEngine, residue_registry: ResidueRegistry, deferred_registry: DeferredWorkRegistry, last_fabric_state: FabricStateModel, last_schedule: MultiScaleSchedule) -> Dict:
        return {
...
class FabricOrchestratorV11:
    def __init__(self, project_id: str = "default_project", session_id: str = "session_001", budget: Optional[BudgetProfile] = None, route_memory: Optional[RouteMemoryEngine] = None, residue_registry: Optional[ResidueRegistry] = None, deferred_registry: Optional[DeferredWorkRegistry] = None, last_fabric_state: Optional[FabricStateModel] = None, last_schedule: Optional[MultiScaleSchedule] = None):
    ...
    def run(self, prompt: str, context_items: List[ContextItem]) -> RuntimeState:
        ...
        state = RuntimeState(prompt=prompt, task_type=task_type, budget=self.budget, project_id=self.project_id, session_id=self.session_id)
        ...
        state.active_modules = [
            "ContinuityPackManager","SessionStateAdapter","FabricOrchestratorV11","PromptClassifier","TokenDifficultyEstimator","DepthGovernor",
            "MemoryTileManager","RouteMemoryEngine","ResidueRegistry","DeferredWorkRegistry","HorizonPlanner","MultiScaleScheduler",
            "SchedulerPressureEngine","ConflictResolver","QueueArbitrationLayer","CandidateRouteGenerator","BranchScoringLayer","EarlyCollapseController",
            "VerificationTrigger",
        ]
...
def demo() -> None:
    ...
    project_a = "project_math_lab"
    session_a1 = "session_001"
    session_a2 = "session_002"
    project_b = "project_runtime_lab"
    session_b1 = "session_001"

    prompt_math = "Prove why a structural contradiction route may be stronger than brute-force checking in a mathematics simulator."
    prompt_runtime = "Summarize how scheduler pressure and queue arbitration stabilize runtime execution."

    print("=== v1.1 / Project A / Session 001 / Fresh Run ===")
    runtime_a1 = FabricOrchestratorV11(project_id=project_a, session_id=session_a1)
    r_a1 = runtime_a1.run(prompt_math, context)
    payload_a1 = SessionStateAdapter.export_payload(project_a, session_a1, runtime_a1.budget, runtime_a1.route_memory, runtime_a1.residue_registry, runtime_a1.deferred_registry, runtime_a1.last_fabric_state, runtime_a1.last_schedule)
    path_a1 = manager.save_session(project_a, session_a1, payload_a1)
    print(f"Saved: {path_a1}")
    print(f"Project: {r_a1.project_id}")
    print(f"Session: {r_a1.session_id}")
    print(f"Chosen route: {r_a1.chosen_route}")
    print(f"Schedule loads: {r_a1.fabric_state.schedule_loads}")

    print("\n=== v1.1 / Project A / Session 002 / Resume from Session 001 ===")
    loaded_a1 = manager.load_session(project_a, session_a1)
    budget_a, route_memory_a, residue_registry_a, deferred_registry_a, last_fabric_state_a, last_schedule_a = SessionStateAdapter.restore(loaded_a1)
    runtime_a2 = FabricOrchestratorV11(
        project_id=project_a,
        session_id=session_a2,
        budget=budget_a,
        route_memory=route_memory_a,
        residue_registry=residue_registry_a,
        deferred_registry=deferred_registry_a,
        last_fabric_state=last_fabric_state_a,
        last_schedule=last_schedule_a,
    )
    r_a2 = runtime_a2.run(prompt_math, context)
    payload_a2 = SessionStateAdapter.export_payload(project_a, session_a2, runtime_a2.budget, runtime_a2.route_memory, runtime_a2.residue_registry, runtime_a2.deferred_registry, runtime_a2.last_fabric_state, runtime_a2.last_schedule)
    path_a2 = manager.save_session(project_a, session_a2, payload_a2)
    print(f"Saved: {path_a2}")
    print(f"Project: {r_a2.project_id}")
    print(f"Session: {r_a2.session_id}")
    print(f"Historical route used: {r_a2.historical_route_used}")
    print(f"Chosen route: {r_a2.chosen_route}")
    print(f"Schedule loads: {r_a2.fabric_state.schedule_loads}")
    print(f"Arbitration count: {r_a2.fabric_state.arbitration_count}")

    print("\n=== v1.1 / Project B / Session 001 / Independent Run ===")
    runtime_b1 = FabricOrchestratorV11(project_id=project_b, session_id=session_b1)
    r_b1 = runtime_b1.run(prompt_runtime, context)
    payload_b1 = SessionStateAdapter.export_payload(project_b, session_b1, runtime_b1.budget, runtime_b1.route_memory, runtime_b1.residue_registry, runtime_b1.deferred_registry, runtime_b1.last_fabric_state, runtime_b1.last_schedule)
    path_b1 = manager.save_session(project_b, session_b1, payload_b1)
    print(f"Saved: {path_b1}")
    print(f"Project: {r_b1.project_id}")
    print(f"Session: {r_b1.session_id}")
    print(f"Chosen route: {r_b1.chosen_route}")
    print(f"Schedule loads: {r_b1.fabric_state.schedule_loads}")

    print("\n=== Continuity Pack Registry ===")
    print(json.dumps({"projects": manager.list_projects(), "sessions_project_a": manager.list_sessions(project_a), "sessions_project_b": manager.list_sessions(project_b)}, indent=2))

    print("\n=== Project A / Session 002 Snapshot ===")
    print(json.dumps(manager.load_session(project_a, session_a2), indent=2))
