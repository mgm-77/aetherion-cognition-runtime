...
VERSION = 'v4.1'
PACK_ROOT_NAME = 'morph_runtime_core_v4_1'
...
MODULES_BY_TASK = {
    'math': ['RuntimeKernel','RuntimeStateManager','RuntimeIndexManager','ContinuityPackManager','SessionStateAdapter',
             ...
             'QuickResumePack','SummaryPackBuilder','PackValidator','IntegrityGuard','FabricOrchestratorV41',
             'RegressionDetector','AnomalyDetector','TrendDetector','SmartResumeEngine','NextBestActionEngine',
             'StatusRegistry','MilestoneManager','StablePromotionEngine'],
...
class MorphRuntimeCoreV41:
    def __init__(self, base_dir: Path):
        ...
        self.report_txt = self.project_dir / 'report_v4.1.txt'
        self.report_md = self.project_dir / 'report_v4.1.md'
        self.resume_txt = self.project_dir / 'smart_resume_v4_1.txt'
        self.resume_json = self.project_dir / 'smart_resume_v4_1.json'
        self.resume_md = self.project_dir / 'smart_resume_v4_1.md'
        self.next_action_txt = self.project_dir / 'next_best_action_v4_1.txt'
        self.next_action_json = self.project_dir / 'next_best_action_v4_1.json'
        self.next_action_md = self.project_dir / 'next_best_action_v4_1.md'
        self.status_txt = self.project_dir / 'status_registry_v4_1.txt'
        self.status_json = self.project_dir / 'status_registry_v4_1.json'
        self.status_md = self.project_dir / 'status_registry_v4_1.md'
        self.milestone_txt = self.project_dir / 'milestones_v4_1.txt'
        self.milestone_json = self.project_dir / 'milestones_v4_1.json'
        self.milestone_md = self.project_dir / 'milestones_v4_1.md'
        self.stable_txt = self.project_dir / 'stable_registry_v4_1.txt'
        self.stable_json = self.project_dir / 'stable_registry_v4_1.json'
        self.stable_md = self.project_dir / 'stable_registry_v4_1.md'
        self.release_txt = self.project_dir / 'release_summary_v4_1.txt'
        self.release_json = self.project_dir / 'release_summary_v4_1.json'
        self.checkpoint_registry_txt = self.project_dir / 'checkpoint_registry_v4_1.txt'
        self.checkpoint_registry_json = self.project_dir / 'checkpoint_registry_v4_1.json'
        self.best_checkpoint_txt = self.project_dir / 'best_checkpoint_v4_1.txt'
        self.best_checkpoint_json = self.project_dir / 'best_checkpoint_v4_1.json'
        self.leaderboard_txt = self.project_dir / 'checkpoint_leaderboard_v4_1.txt'
        self.leaderboard_json = self.project_dir / 'checkpoint_leaderboard_v4_1.json'
        self.regression_txt = self.project_dir / 'regression_report_v4_1.txt'
        self.regression_json = self.project_dir / 'regression_report_v4_1.json'
        self.anomaly_txt = self.project_dir / 'anomaly_report_v4_1.txt'
        self.anomaly_json = self.project_dir / 'anomaly_report_v4_1.json'
        self.trend_txt = self.project_dir / 'trend_report_v4_1.txt'
        self.trend_json = self.project_dir / 'trend_report_v4_1.json'
    ...
    def build_smart_resume(self):
        ...
        if rec:
            if rec['branch'] == 'main':
                cmd = f"python morph_runtime_core_v4_1.py --append-task {rec['task_type']} --resume-session {rec['session_id']}"
            else:
                cmd = f"python morph_runtime_core_v4_1.py --append-task {rec['task_type']} --resume-session {rec['session_id']} --branch {rec['branch']}"
        ...
        write_text(self.resume_md, '# Smart Resume v4.1\n\n' + '\n'.join(f'- {ln}' for ln in lines if ln.strip()) + '\n')
        return payload
    ...
    def build_next_best_action(self):
        ...
        actions = []
        if anomalies:
            actions.append({'priority': 1,'action': 'investigate_anomalies','reason': f'{anomalies} anomaly items detected','suggested_command': 'python morph_runtime_core_v4_1.py --report-only'})
        if any(r.get('verdict') == 'regression-risk' for r in regressions):
            actions.append({'priority': 2,'action': 'review_regression_risk','reason': 'one or more branches show regression risk','suggested_command': 'python morph_runtime_core_v4_1.py --report-only'})
        ...
        if not actions:
            actions.append({'priority': 1,'action': 'create_stable_checkpoint','reason': 'system is stable and no issues were detected','suggested_command': 'python morph_runtime_core_v4_1.py --checkpoint-label stable_v4_1'})
        ...
        write_text(self.next_action_md, '# Next Best Action v4.1\n\n' + '\n'.join(f'- {ln}' for ln in lines if ln.strip()) + '\n')
        return payload
    ...
    def build_status_registry(self):
        ...
        write_text(self.status_md, '# Status Registry v4.1\n\n' + '\n'.join(f'- {ln}' for ln in lines if ln.strip()) + '\n')
        return payload
    ...
    def build_milestones(self):
        ...
        write_text(self.milestone_md, '# Milestones v4.1\n\n' + '\n'.join(f'- {ln}' for ln in lines if ln.strip()) + '\n')
        return payload
    ...
    def build_stable_registry(self):
        ...
        write_text(self.stable_md, '# Stable Registry v4.1\n\n' + '\n'.join(f'- {ln}' for ln in lines if ln.strip()) + '\n')
        return payload
    ...
    def print_summary(self, session, root_created):
        print('=== MORPH Runtime Core v4.1 / Summary ===')
        ...
        print(f'Status txt:     {self.status_txt}')
        print(f'Milestone txt:  {self.milestone_txt}')
        print(f'Stable txt:     {self.stable_txt}')
        print(f'ZIP export:     {self.zip_export_file}')
        print()
        print('=== Status Snapshot ===')
        print(json.dumps({'session_id': session['session_id'],'parent': session['parent'],'branch': session['branch'],'task_type': session['task_type'],'chosen_route': session['chosen_route'],'verification_armed': session['verification_armed'],'route_confidence': session['route_confidence'],'deferred_item_count': session['deferred_item_count'],'health_score': session['health_score'],'saved_at': session['saved_at']}, indent=2))

    def cli(self):
        parser = argparse.ArgumentParser(description='MORPH Runtime Core v4.1')
        ...
        if args.report_only:
            ...
            print(self.report_txt); print(self.resume_txt); print(self.next_action_txt); print(self.status_txt); print(self.milestone_txt); print(self.stable_txt); print(self.release_txt)
            return
...
def main():
    MorphRuntimeCoreV41(Path.cwd()).cli()
