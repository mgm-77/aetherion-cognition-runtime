...
VERSION = "v4.6"
PACK_ROOT_NAME = "morph_runtime_core_v4_6"
...
class MorphRuntimeCoreV46:
    def __init__(self, base_dir: Path):
        ...
        self.report_txt = self.project_dir / "report_v4.6.txt"
        self.report_md = self.project_dir / "report_v4.6.md"
        self.historical_txt = self.project_dir / "historical_target_v4_6.txt"
        self.historical_json = self.project_dir / "historical_target_v4_6.json"
        self.current_txt = self.project_dir / "current_target_v4_6.txt"
        self.current_json = self.project_dir / "current_target_v4_6.json"
        self.recent_txt = self.project_dir / "recent_target_v4_6.txt"
        self.recent_json = self.project_dir / "recent_target_v4_6.json"
        self.conflict_txt = self.project_dir / "target_conflict_v4_6.txt"
        self.conflict_json = self.project_dir / "target_conflict_v4_6.json"
        self.switch_txt = self.project_dir / "target_switch_v4_6.txt"
        self.switch_json = self.project_dir / "target_switch_v4_6.json"
        self.switch_md = self.project_dir / "target_switch_v4_6.md"
    ...
    def build_upload_reports(self):
        ...
        txt_lines = [
            ...
            "- target switch recommendation",
            "- when to stay current vs switch",
            "- historical/current/recent bridge logic",
        ]
        write_text(self.report_txt, "\n".join(txt_lines) + "\n")
        write_text(self.report_md, "# Report v4.6\n\n" + "\n".join(f"- {x}" for x in txt_lines if x.strip()) + "\n")
    ...
    def print_summary(self, session, root_created):
        print("=== MORPH Runtime Core v4.6 / Summary ===")
        ...
        print(f"Conflict txt:     {self.conflict_txt}")
        print(f"Switch txt:       {self.switch_txt}")
        print(f"ZIP export:       {self.zip_export_file}")

    def cli(self):
        parser = argparse.ArgumentParser(description="MORPH Runtime Core v4.6")
        ...
        parser.add_argument("--target-conflict-only", action="store_true")
        parser.add_argument("--target-switch-only", action="store_true")
        ...
        if args.report_only:
            ...
            print(self.conflict_txt)
            print(self.switch_txt)
            return
        ...
        if args.target_conflict_only:
            ...
            self.build_conflict_detector()
            print(self.conflict_txt)
            print(self.conflict_json)
            return

        if args.target_switch_only:
            self.init_pack()
            self.build_historical_target()
            self.build_current_target()
            self.build_recent_target()
...
def main():
    MorphRuntimeCoreV46(Path.cwd()).cli()
