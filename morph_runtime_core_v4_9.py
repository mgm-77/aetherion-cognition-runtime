...
VERSION = "v4.9"
PACK_ROOT_NAME = "morph_runtime_core_v4_9"
...
class MorphRuntimeCoreV49:
    def __init__(self, base_dir: Path):
        ...
        self.report_txt = self.project_dir / "report_v4.9.txt"
        self.report_md = self.project_dir / "report_v4.9.md"

        self.historical_txt = self.project_dir / "historical_target_v4_9.txt"
        self.historical_json = self.project_dir / "historical_target_v4_9.json"
        self.current_txt = self.project_dir / "current_target_v4_9.txt"
        self.current_json = self.project_dir / "current_target_v4_9.json"
        self.recent_txt = self.project_dir / "recent_target_v4_9.txt"
        self.recent_json = self.project_dir / "recent_target_v4_9.json"

        self.conflict_txt = self.project_dir / "target_conflict_v4_9.txt"
        self.conflict_json = self.project_dir / "target_conflict_v4_9.json"

        self.switch_txt = self.project_dir / "target_switch_v4_9.txt"
        self.switch_json = self.project_dir / "target_switch_v4_9.json"
        self.switch_md = self.project_dir / "target_switch_v4_9.md"

        self.lock_txt = self.project_dir / "target_lock_v4_9.txt"
        self.lock_json = self.project_dir / "target_lock_v4_9.json"

        self.protection_txt = self.project_dir / "target_protection_v4_9.txt"
        self.protection_json = self.project_dir / "target_protection_v4_9.json"

        self.drift_alert_txt = self.project_dir / "target_drift_alert_v4_9.txt"
        self.drift_alert_json = self.project_dir / "target_drift_alert_v4_9.json"

        self.recovery_route_txt = self.project_dir / "recovery_route_v4_9.txt"
        self.recovery_route_json = self.project_dir / "recovery_route_v4_9.json"
        self.recovery_route_md = self.project_dir / "recovery_route_v4_9.md"

        self.auto_return_txt = self.project_dir / "auto_return_v4_9.txt"
        self.auto_return_json = self.project_dir / "auto_return_v4_9.json"
        self.auto_return_md = self.project_dir / "auto_return_v4_9.md"

        self.safe_resume_txt = self.project_dir / "safe_resume_path_v4_9.txt"
        self.safe_resume_json = self.project_dir / "safe_resume_path_v4_9.json"
        self.safe_resume_md = self.project_dir / "safe_resume_path_v4_9.md"

        self.recovery_checkpoint_txt = self.project_dir / "recovery_checkpoint_v4_9.txt"
        self.recovery_checkpoint_json = self.project_dir / "recovery_checkpoint_v4_9.json"
        self.recovery_checkpoint_md = self.project_dir / "recovery_checkpoint_v4_9.md"

        self.guarded_resume_txt = self.project_dir / "guarded_resume_v4_9.txt"
        self.guarded_resume_json = self.project_dir / "guarded_resume_v4_9.json"
        self.guarded_resume_md = self.project_dir / "guarded_resume_v4_9.md"

        self.confirm_switch_txt = self.project_dir / "confirm_before_switch_v4_9.txt"
        self.confirm_switch_json = self.project_dir / "confirm_before_switch_v4_9.json"
        self.confirm_switch_md = self.project_dir / "confirm_before_switch_v4_9.md"
    ...
    def load_index(self):
        ...
        idx.setdefault("recovery_checkpoints", [])
        return idx
    ...
    def build_switch_recommendation(self):
        ...
        write_text(self.switch_md, "# Target Switch v4.9\n\n" + "\n".join(f"- {x}" for x in lines) + "\n")
        return payload
    ...
    def build_recovery_route(self):
        switch = read_json(self.switch_json, default={})
        conflict = read_json(self.conflict_json, default={})
        recovery_mode = "stay_on_current"
        route = None
        session_id = None
        command = None

        if switch.get("recommendation") == "switch_to_historical":
            recovery_mode = "return_to_historical"
            session_id = switch.get("target_value")
        elif switch.get("recommendation") == "switch_to_recent":
            recovery_mode = "return_to_recent"
            session_id = switch.get("target_value")
        else:
            current = read_json(self.current_json, default={}).get("current_target")
        ...
        if session_id:
            ...
            s = read_json(Path(meta["path"]), default={}) if meta else {}
            route = s.get("chosen_route")
            task = s.get("task_type")
            branch = s.get("branch")
            if branch == "main":
                command = f"python morph_runtime_core_v4_9.py --append-task {task} --resume-session {session_id}"
            else:
                command = f"python morph_runtime_core_v4_9.py --append-task {task} --resume-session {session_id} --branch {branch}"
        ...
        write_json(self.recovery_route_json, payload)
        lines = [
            f"Recovery Route {VERSION}",
            f"Project: {self.project_id}",
            f"Recovery mode: {payload['recovery_mode']}",
            f"Recovery session: {payload['recovery_session']}",
            f"Recovery route: {payload['recovery_route']}",
            f"Resume command: {payload['resume_command']}",
            f"Conflict detected: {payload['conflict_detected']}",
        ]
        write_text(self.recovery_route_txt, "\n".join(lines) + "\n")
        write_text(self.recovery_route_md, "# Recovery Route v4.9\n\n" + "\n".join(f"- {x}" for x in lines) + "\n")
        return payload

    def build_auto_return(self):
        drift = read_json(self.drift_alert_json, default={})
        recovery = read_json(self.recovery_route_json, default={})
        action = "hold_position"
        reason = "no forced return required"

        if drift.get("drift_alert") and recovery.get("recovery_session"):
            action = "auto_return_recommended"
            reason = "drift alert is active and a safe recovery session exists"
        ...
        payload = {
            ...
            "action": action,
            "reason": reason,
            "recovery_session": recovery.get("recovery_session"),
            "resume_command": recovery.get("resume_command"),
        }
        write_json(self.auto_return_json, payload)
        lines = [
            f"Auto Return {VERSION}",
            f"Project: {self.project_id}",
        ...
        write_text(self.auto_return_txt, "\n".join(lines) + "\n")
        write_text(self.auto_return_md, "# Auto Return v4.9\n\n" + "\n".join(f"- {x}" for x in lines) + "\n")
        return payload

    def build_safe_resume_path(self):
        recovery = read_json(self.recovery_route_json, default={})
        auto_return = read_json(self.auto_return_json, default={})
        index = self.load_index()
        path_sessions = []

        target = recovery.get("recovery_session")
        while target and target in index.get("sessions", {}):
            meta = index["sessions"][target]
            path_sessions.append(target)
            target = meta.get("parent")
        path_sessions.reverse()
        ...
        payload = {
            ...
            "resume_command": recovery.get("resume_command"),
            "auto_return_action": auto_return.get("action"),
        }
        write_json(self.safe_resume_json, payload)
        lines = [
            f"Safe Resume Path {VERSION}",
            f"Project: {self.project_id}",
            f"Auto return action: {payload['auto_return_action']}",
            f"Resume command: {payload['resume_command']}",
            "Path sessions:",
        ]
        for sid in path_sessions:
            lines.append(f"- {sid}")
        write_text(self.safe_resume_txt, "\n".join(lines) + "\n")
        write_text(self.safe_resume_md, "# Safe Resume Path v4.9\n\n" + "\n".join(f"- {x}" for x in lines) + "\n")
        return payload

    def create_recovery_checkpoint(self):
        recovery = read_json(self.recovery_route_json, default={})
        session_id = recovery.get("recovery_session")
        if not session_id:
            payload = {"version": VERSION, "project_id": self.project_id, "generated_at": now_iso(), "created": False, "reason": "no_recovery_session"}
            write_json(self.recovery_checkpoint_json, payload)
            write_text(self.recovery_checkpoint_txt, "Recovery Checkpoint v4.9\nProject: project_math_lab\nCreated: False\nReason: no_recovery_session\n")
            write_text(self.recovery_checkpoint_md, "# Recovery Checkpoint v4.9\n\n- Created: False\n- Reason: no_recovery_session\n")
            return payload

        index = self.load_index()
        meta = index.get("sessions", {}).get(session_id)
        session = read_json(Path(meta["path"]), default={}) if meta else {}
        label = f"recovery_{session_id}"
        ...
        payload = {
            ...
            "recovery_session": session_id,
            "recovery_route": session.get("chosen_route"),
        }
        write_json(cp_path, {"label": label, "saved_at": payload["generated_at"], "session": session})
        index["recovery_checkpoints"].append({
            "label": label,
            "checkpoint_path": str(cp_path),
            "session_id": session_id,
        ...
        write_text(self.recovery_checkpoint_txt, "\n".join(lines) + "\n")
        write_text(self.recovery_checkpoint_md, "# Recovery Checkpoint v4.9\n\n" + "\n".join(f"- {x}" for x in lines) + "\n")
        return payload

    def build_guarded_resume(self):
        protection = read_json(self.protection_json, default={}).get("target_protection", {})
        recovery = read_json(self.recovery_route_json, default={})
        switch = read_json(self.switch_json, default={})
        guarded = False
        mode = "normal_resume"
        reason = "no extra guard required"
        command = recovery.get("resume_command")

        if protection.get("enabled") and switch.get("recommendation") in ("switch_to_historical", "switch_to_recent"):
            guarded = True
            mode = "guarded_resume"
            reason = "protected target exists and switch recommendation is active"
        ...
        write_json(self.guarded_resume_json, payload)
        lines = [
            f"Guarded Resume {VERSION}",
            f"Project: {self.project_id}",
            f"Guarded: {payload['guarded']}",
            f"Mode: {payload['mode']}",
            f"Reason: {payload['reason']}",
            f"Resume command: {payload['resume_command']}",
        ]
        write_text(self.guarded_resume_txt, "\n".join(lines) + "\n")
        write_text(self.guarded_resume_md, "# Guarded Resume v4.9\n\n" + "\n".join(f"- {x}" for x in lines) + "\n")
        return payload

    def build_confirm_before_switch(self):
        switch = read_json(self.switch_json, default={})
        drift = read_json(self.drift_alert_json, default={})
        confirm = False
        reason = "auto hold not required"

        if drift.get("drift_alert") and switch.get("recommendation") in ("switch_to_historical", "switch_to_recent"):
            confirm = True
            reason = "runtime should ask for confirmation before switching target"

        payload = {
            "version": VERSION,
            "project_id": self.project_id,
            "generated_at": now_iso(),
            "confirm_before_switch": confirm,
            "reason": reason,
            "recommended_target": switch.get("target_value"),
            "recommended_type": switch.get("target_type"),
        }
        write_json(self.confirm_switch_json, payload)
        lines = [
            f"Confirm Before Switch {VERSION}",
            f"Project: {self.project_id}",
            f"Confirm before switch: {payload['confirm_before_switch']}",
            f"Reason: {payload['reason']}",
            f"Recommended target: {payload['recommended_target']}",
            f"Recommended type: {payload['recommended_type']}",
        ]
        write_text(self.confirm_switch_txt, "\n".join(lines) + "\n")
        write_text(self.confirm_switch_md, "# Confirm Before Switch v4.9\n\n" + "\n".join(f"- {x}" for x in lines) + "\n")
        return payload
    ...
    def build_upload_reports(self):
        ...
        txt_lines = [
            ...
            "- recovery checkpoint",
            "- guarded resume",
            "- confirm-before-switch logic",
            "- safer return control",
        ]
        write_text(self.report_txt, "\n".join(txt_lines) + "\n")
        write_text(self.report_md, "# Report v4.9\n\n" + "\n".join(f"- {x}" for x in txt_lines if x.strip()) + "\n")
    ...
    def build_all_reports(self):
        ...
        self.build_recovery_route()
        self.build_auto_return()
        self.build_safe_resume_path()
        self.build_guarded_resume()
        self.build_confirm_before_switch()
        self.build_upload_reports()
        self.build_zip()

    def print_summary(self, session, root_created):
        print("=== MORPH Runtime Core v4.9 / Summary ===")
        ...
        print(f"Recovery checkpoint:   {self.recovery_checkpoint_txt}")
        print(f"Guarded resume txt:    {self.guarded_resume_txt}")
        print(f"Confirm switch txt:    {self.confirm_switch_txt}")
        print(f"ZIP export:            {self.zip_export_file}")

    def cli(self):
        parser = argparse.ArgumentParser(description="MORPH Runtime Core v4.9")
        ...
        parser.add_argument("--recovery-checkpoint-only", action="store_true")
        parser.add_argument("--guarded-resume-only", action="store_true")
        parser.add_argument("--confirm-before-switch-only", action="store_true")
        ...
        if args.report_only:
            ...
            print(self.recovery_checkpoint_txt)
            print(self.guarded_resume_txt)
            print(self.confirm_switch_txt)
            return

        if args.recovery_checkpoint_only:
            self.init_pack()
            self.build_all_reports()
            self.create_recovery_checkpoint()
            print(self.recovery_checkpoint_txt)
            print(self.recovery_checkpoint_json)
            return

        if args.guarded_resume_only:
            self.init_pack()
            self.build_all_reports()
            print(self.guarded_resume_txt)
            print(self.guarded_resume_json)
            return

        if args.confirm_before_switch_only:
            self.init_pack()
            self.build_all_reports()
            print(self.confirm_switch_txt)
            print(self.confirm_switch_json)
            return
...
def main():
    MorphRuntimeCoreV49(Path.cwd()).cli()
