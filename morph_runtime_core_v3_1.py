...
VERSION = 'v3.1'
PACK_ROOT_NAME = 'morph_runtime_core_v3_1'
...
MODULES_BY_TASK = {
    'math': [
        ...
        'QuickResumePack','SummaryPackBuilder','PackValidator','IntegrityGuard','FabricOrchestratorV31',
        'MergeIntelligenceEngine','CompareModesEngine','BranchVerdictEngine','StateInspector','RuntimeDashboardBuilder',
        'CheckpointManager','ReleaseReportBuilder','SnapshotExporter','SessionQueryEngine'
    ],
    'simple': [
        'RuntimeKernel','RuntimeStateManager','RuntimeIndexManager','ContinuityPackManager','SessionStateAdapter',
        'RecoveryEngine','AutoRepairBasic','OrphanHandler','ComparativeResumeEngine','HandoffExporter',
...
class MorphRuntimeCoreV31:
    def __init__(self, base_dir: Path):
        ...
        self.branch_compare_file = self.project_dir / 'branch_compare_v3_1.json'
        self.branch_verdict_md_file = self.project_dir / 'branch_verdict_v3_1.md'
        self.branch_verdict_txt_file = self.project_dir / 'branch_verdict_v3_1.txt'
        self.runtime_dashboard_json = self.project_dir / 'runtime_dashboard_v3_1.json'
        self.runtime_dashboard_txt = self.project_dir / 'runtime_dashboard_v3_1.txt'
        self.runtime_dashboard_md = self.project_dir / 'runtime_dashboard_v3_1.md'
        self.archive_summary_file = self.project_dir / 'archive_summary.json'
        self.release_summary_json = self.project_dir / 'release_summary_v3_1.json'
        self.release_summary_txt = self.project_dir / 'release_summary_v3_1.txt'
        self.report_txt = self.project_dir / 'report_v3.1.txt'
        self.report_md = self.project_dir / 'report_v3.1.md'
        self.session_query_json = self.project_dir / 'session_query_v3_1.json'
        self.session_query_txt = self.project_dir / 'session_query_v3_1.txt'
    ...
    def build_compact_handoff(self):
        ...
        data = {
            ...
            'resume_main': f"python morph_runtime_core_v3_1.py --append-task math --resume-session {latest['session_id']}" if latest else None,
            'resume_simple': "python morph_runtime_core_v3_1.py --append-task simple --branch simple_track" if latest else None,
        }
    ...
    def build_quick_resume_pack(self):
        ...
        data = {
            ...
            'main_resume': f"python morph_runtime_core_v3_1.py --append-task math --resume-session {latest['session_id']}" if latest and latest['task_type'] == 'math' else None,
            'simple_resume': "python morph_runtime_core_v3_1.py --append-task simple --branch simple_track",
            'checkpoint_create': "python morph_runtime_core_v3_1.py --checkpoint-label stable_v3_1",
    ...
    def branch_verdicts(self):
        ...
        md_lines = ['# Branch Verdicts v3.1', '']
        txt_lines = ['Branch Verdicts v3.1', '']
    ...
    def build_query_results(self, task_type=None, route=None, branch=None, min_health=None, max_deferred=None):
        ...
        for r in results:
            lines.append(
                f"- {r['session_id']} | branch={r['branch']} | task={r['task_type']} | route={r['chosen_route']} | "
                f"health={r['health_score']} | deferred={r['deferred_item_count']} | conf={r['route_confidence']} | saved_at={r['saved_at']}"
            )
    ...
    def build_upload_reports(self):
        ...
        txt_lines = [
            ...
            'V3.1 features:',
            '- inspector queries',
            '- session filtering by task / route / branch / health / deferred',
            '- query export txt/json',
        ]
        write_text(self.report_txt, '\n'.join(txt_lines) + '\n')
        write_text(self.report_md, '# Report v3.1\n\n' + '\n'.join(f'- {line}' for line in txt_lines if line.strip()) + '\n')
    ...
    def print_summary(self, session, root_created):
        print('=== MORPH Runtime Core v3.1 / Summary ===')
        ...
        print(f'Query txt:      {self.session_query_txt}')
        print(f'Query json:     {self.session_query_json}')
    ...
    def cli(self):
        parser = argparse.ArgumentParser(description='MORPH Runtime Core v3.1')
...
def main():
    MorphRuntimeCoreV31(Path.cwd()).cli()
