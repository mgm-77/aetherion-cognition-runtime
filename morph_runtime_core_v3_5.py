...
VERSION = 'v3.5'
PACK_ROOT_NAME = 'morph_runtime_core_v3_5'
...
MODULES_BY_TASK = {
    'math': [
        ...
        'QuickResumePack','SummaryPackBuilder','PackValidator','IntegrityGuard','FabricOrchestratorV35',
        ...
        'PressureSignalBuilder','DriftNotesBuilder','CheckpointDiffBuilder'
    ],
    'simple': [
        'RuntimeKernel','RuntimeStateManager','RuntimeIndexManager','ContinuityPackManager','SessionStateAdapter',
        'RecoveryEngine','AutoRepairBasic','OrphanHandler','ComparativeResumeEngine','HandoffExporter',
...
class MorphRuntimeCoreV35:
    def __init__(self, base_dir: Path):
        ...
        self.branch_compare_file = self.project_dir / 'branch_compare_v3_5.json'
        self.branch_verdict_md_file = self.project_dir / 'branch_verdict_v3_5.md'
        self.branch_verdict_txt_file = self.project_dir / 'branch_verdict_v3_5.txt'
        self.runtime_dashboard_json = self.project_dir / 'runtime_dashboard_v3_5.json'
        self.runtime_dashboard_txt = self.project_dir / 'runtime_dashboard_v3_5.txt'
        self.runtime_dashboard_md = self.project_dir / 'runtime_dashboard_v3_5.md'
        self.archive_summary_file = self.project_dir / 'archive_summary.json'
        self.release_summary_json = self.project_dir / 'release_summary_v3_5.json'
        self.release_summary_txt = self.project_dir / 'release_summary_v3_5.txt'
        self.report_txt = self.project_dir / 'report_v3.5.txt'
        self.report_md = self.project_dir / 'report_v3.5.md'
        self.session_query_json = self.project_dir / 'session_query_v3_5.json'
        self.session_query_txt = self.project_dir / 'session_query_v3_5.txt'
        self.timeline_json = self.project_dir / 'history_timeline_v3_5.json'
        self.timeline_txt = self.project_dir / 'history_timeline_v3_5.txt'
        self.timeline_md = self.project_dir / 'history_timeline_v3_5.md'
        self.pressure_json = self.project_dir / 'pressure_signals_v3_5.json'
        self.pressure_txt = self.project_dir / 'pressure_signals_v3_5.txt'
        self.pressure_md = self.project_dir / 'pressure_signals_v3_5.md'
        self.drift_json = self.project_dir / 'drift_notes_v3_5.json'
        self.drift_txt = self.project_dir / 'drift_notes_v3_5.txt'
        self.drift_md = self.project_dir / 'drift_notes_v3_5.md'
        self.checkpoint_diff_json = self.project_dir / 'checkpoint_diff_v3_5.json'
        self.checkpoint_diff_txt = self.project_dir / 'checkpoint_diff_v3_5.txt'
        self.checkpoint_diff_md = self.project_dir / 'checkpoint_diff_v3_5.md'
    ...
    def build_compact_handoff(self):
        ...
        data = {
            ...
            'resume_main': f"python morph_runtime_core_v3_5.py --append-task math --resume-session {latest['session_id']}" if latest else None,
            'resume_simple': "python morph_runtime_core_v3_5.py --append-task simple --branch simple_track" if latest else None,
        }
    ...
    def build_quick_resume_pack(self):
        ...
        data = {
            ...
            'main_resume': f"python morph_runtime_core_v3_5.py --append-task math --resume-session {latest['session_id']}" if latest and latest['task_type'] == 'math' else None,
            'simple_resume': "python morph_runtime_core_v3_5.py --append-task simple --branch simple_track",
            'checkpoint_create': "python morph_runtime_core_v3_5.py --checkpoint-label stable_v3_5",
    ...
    def branch_verdicts(self):
        ...
        md_lines = ['# Branch Verdicts v3.5', '']
        txt_lines = ['Branch Verdicts v3.5', '']
    ...
    def build_timeline(self):
        ...
        md = ['# History Timeline v3.5', '', f'- **Project:** {self.project_id}', f'- **Entry count:** `{len(entries)}`', '', '## Entries']
        for e in entries:
            if e['type'] == 'session':
    ...
    def build_pressure_signals(self):
        ...
        md = ['# Pressure Signals v3.5', '', f'- **Project:** {self.project_id}', f'- **Signal count:** `{len(signals)}`', '', '## Signals']
        for s in signals:
    ...
    def build_drift_notes(self):
        ...
        md = ['# Drift Notes v3.5', '', f'- **Project:** {self.project_id}', f'- **Branch count:** `{len(notes)}`', '', '## Notes']
        for n in notes:
    ...
    def build_checkpoint_diff(self):
        index = self.load_index()
    ...
    def build_upload_reports(self):
        ...
        txt_lines = [
            ...
            'V3.5 features:',
            '- checkpoint diff',
            '- compare last 2 checkpoints',
            '- checkpoint diff export txt / json / md',
        ]
        write_text(self.report_txt, '\n'.join(txt_lines) + '\n')
        write_text(self.report_md, '# Report v3.5\n\n' + '\n'.join(f'- {line}' for line in txt_lines if line.strip()) + '\n')
    ...
    def print_summary(self, session, root_created):
        print('=== MORPH Runtime Core v3.5 / Summary ===')
        ...
        print(f'Checkpoint txt: {self.checkpoint_diff_txt}')
        print(f'Checkpoint md:  {self.checkpoint_diff_md}')
        print(f'Checkpoint json:{self.checkpoint_diff_json}')
    ...
    def cli(self):
        parser = argparse.ArgumentParser(description='MORPH Runtime Core v3.5')
        ...
        if args.checkpoint_diff_only:
            self.init_pack()
            payload = self.build_checkpoint_diff()
            print(json.dumps(payload, indent=2, ensure_ascii=False))
            print(self.checkpoint_diff_txt)
...
def main():
    MorphRuntimeCoreV35(Path.cwd()).cli()
